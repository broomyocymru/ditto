...
import requests
from ditto.core import util, logger, config


class DittoAnalytics(object):
    def __init__(self, **kwargs):
        self.data = {
            'timestamp': ("%s" % datetime.datetime.now()),
            'ditto_version': util.ditto_version(),
...
class MockAnalytics(DittoAnalytics):
    def __init__(self, **kwargs):
        DittoAnalytics.__init__(self)

    def submit(self, data=None):
        pass


class Splunk(DittoAnalytics):
    def __init__(self, **kwargs):
        DittoAnalytics.__init__(self)
